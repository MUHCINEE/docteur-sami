tailwind.config={darkMode:'class',theme:{extend:{colors:{cream:'#FBF8F2',sand:'#F1ECE1',forest:{50:'#E8F9F8',100:'#C2F3F0',300:'#7FE4E0',500:'#48D1CC',600:'#3BB3AF',700:'#2A8683',900:'#164846'},clay:'#C08552',ink:'#2B2A26',},fontFamily:{serif:['Fraunces','serif'],sans:['Manrope','sans-serif'],arabic:['Noto Naskh Arabic','serif'],},}}}
...
initRdvForm();initThemeToggle()});function initThemeToggle(){const root=document.documentElement;const toggles=document.querySelectorAll('[data-theme-toggle]');if(!toggles.length)return;let transitionTimer=null;function applyTheme(theme,persist){root.classList.add('theme-transition');root.classList.toggle('dark',theme==='dark');toggles.forEach(function(btn){btn.setAttribute('aria-checked',String(theme==='dark'))});if(persist){try{localStorage.setItem('theme',theme)}catch(e){}}
clearTimeout(transitionTimer);transitionTimer=setTimeout(function(){root.classList.remove('theme-transition')},400)}
toggles.forEach(function(btn){btn.setAttribute('aria-checked',String(root.classList.contains('dark')));btn.addEventListener('click',function(){const isDark=root.classList.contains('dark');applyTheme(isDark?'light':'dark',!0)})});if(window.matchMedia){window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change',function(e){let saved=null;try{saved=localStorage.getItem('theme')}catch(err){}
if(!saved){applyTheme(e.matches?'dark':'light',!1)}})}}
function initRdvForm(){const form=document.getElementById('rdv-form');if(!form)return;const dateInput=document.getElementById('rdv-date');if(dateInput&&typeof flatpickr!=='undefined'){const fp=flatpickr(dateInput,{locale:'fr',dateFormat:'d/m/Y',minDate:'today',disableMobile:!0,animate:!0});const trigger=document.getElementById('calendar-trigger');if(trigger){trigger.addEventListener('click',function(){fp.open()})}}
